...
def flatten_mdp(env, policy):
    """Achata o MDP para um Processo de Recompensa de Markov (MRP)[cite: 98]."""
    P_pi = np.zeros([env.nb_states, env.nb_states]) # Matriz de prob de transição [cite: 100]
    R_pi = np.zeros([env.nb_states]) # Recompensa esperada [cite: 102]
    
    for s in range(env.nb_states): # [cite: 104]
        for a in range(env.nb_actions): # [cite: 105]
            for p, s_, r, is_term in env.model[s][a]: # [cite: 107, 113, 114]
                P_pi[s, s_] += policy[s, a] * p # [cite: 115]
                R_pi[s] += policy[s, a] * p * r # [cite: 116]
                
    return P_pi, R_pi # [cite: 121]

def iter_policy_eval(env, policy, gamma, theta):
    """Implementação Naive (com loops) do Iterative Policy Evaluation[cite: 21, 23]."""
    V = np.zeros(env.nb_states) # [cite: 40]
    while True: # [cite: 41]
        delta = 0 # [cite: 42]
        for s in range(env.nb_states): # [cite: 43]
            v = V[s] # [cite: 43]
            tmp = 0 # [cite: 45]
            for a in range(env.nb_actions): # [cite: 46]
                for p, s_, r, is_term in env.model[s][a]: # [cite: 47, 51]
                    tmp += policy[s, a] * p * (r + gamma * V[s_]) # [cite: 54]
            V[s] = tmp # [cite: 55]
            delta = max(delta, abs(v - V[s])) # [cite: 56]
        if delta < theta: break # [cite: 57]
    return V # [cite: 58]
